...
export interface DeployedAddresses {
  wormRouter: Record<number, string>;
  tokenBridgeHelpers: Record<number, string>;
  erc20s: Record<number, string[]>;
}

export function getWormRouter(chainId: number): WormRouter {
  const deployed = loadDeployedAddresses().wormRouter[chainId];
  if (!deployed) {
    throw new Error(`No deployed worm router on chain ${chainId}`);
  }
  return WormRouter__factory.connect(deployed, getWallet(chainId));
}

export function getTokenBridgeHelpers(chainId: number): TokenBridgeHelpers {
  const deployed = loadDeployedAddresses().tokenBridgeHelpers[chainId];
  if (!deployed) {
    throw new Error(`No deployed token bridge helper on chain ${chainId}`);
  }
  return TokenBridgeHelpers__factory.connect(deployed, getWallet(chainId));
}
