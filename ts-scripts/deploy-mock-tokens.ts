...
export async function deployMockToken() {
  ...
  const TT = await new ERC20Mock__factory(signer).deploy("TestToken", "TT");
  await TT.deployed();
  console.log(`TT deployed to ${TT.address} on chain ${from.chainId}`);
  deployed.erc20s[6] = [TT.address];

  console.log("Minting...");
  await TT.mint(signer.address, ethers.utils.parseEther("10")).then(wait);
  console.log("Minted 10 TT to signer");
  ...
  for (const chain of loadConfig().chains) {
    ...
    await attestWorkflow({ from: getChain(6), to: chain, token: TT.address });
  }
